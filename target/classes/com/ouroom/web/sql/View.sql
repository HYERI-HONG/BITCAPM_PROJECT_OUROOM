...
SELECT A.*, COUNT(c.SEQ) AS COMMENT_CNT
	FROM (
		SELECT
			ROW_NUMBER() OVER(ORDER BY p.SEQ DESC) AS No, p.*, m.NICKNAME, m.PROFILE, COUNT(i.SEQ) AS IMGTAG_CNT
		FROM post p
...
SELECT p.*, m.NICKNAME, m.PROFILE, COUNT(c.SEQ) AS COMMENT_CNT
FROM post p
	LEFT JOIN member m ON p.MEM_SEQ LIKE m.SEQ
...
FROM(
	SELECT 
		ROW_NUMBER() OVER(ORDER BY c.SEQ DESC) AS No, c.SEQ, c.COMMENT, c.ARTICLE_SEQ AS POST_SEQ, m.NICKNAME, m.PROFILE 
	FROM 
		comment c
