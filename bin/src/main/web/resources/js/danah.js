...
const danah = (()=>{
	...
	let setContentView =()=>{
		...
		$('#h_search_btn').attr({style : "visibility: visible;font-size:25px; margin-bottom:9px; vertical-align: bottom; margin-left:10px"}).click(d=>{
				d.preventDefault();
				danah.util.search();

			});
		$('#h_wirte_btn').attr({style : "visibility: visible; top:12px"}).click(d=>{
				d.preventDefault();
...
danah.service = (()=>{
	...
	let list = d=>{
		...
		for (let i = 0; i <= 20; i++) {
			...
			a.children(':first')
			.append(
				danah.compo.a({c: 'd_post_writer_profile', hr:'#', ht: danah.compo.img({s:$img+'/danah/profile.jpeg',a:'의 프로필 사진'})}),
			...
			.append(
				...
				.append(
					...
					danah.compo.img({c: 'horizon', s: $img+'/danah/post/1.jpeg'}),
					// 정보가 있을때 생성
	...
	filter: ()=>{
		...
		$.each([{k: '정렬', v: '인기순'}, {k: '공간', v: '모든공간'}, {k: '평수', v: '모든평수'}, {k: '제품정보',v: '모두'}], (i,v)=>{
			...
			.append(
				...
				.append(
					danah.compo.span({c: 'd_filter_item_btn_search_key'})
					.text(v.k)
					...
					danah.compo.span({c: 'd_filter_item_btn_search_val'})
					.text(v.v)
				)
			)
				...
				if(l.eq(i).hasClass('d_filter_item_active')){
					...
					.append(
						$.each((i==0 ?
							...
							.append(
								danah.compo.a({c: 'd_filter_item_btn'})
								.text(this)
							)
...
danah.compo = {
	...
	img: d=>{return $('<img/>').addClass(d.c).attr({id: d.i, src: d.s, alt :d.a});},
	i: d=>{return $('<i/>').addClass(d.c).attr({id: d.i});}
};
