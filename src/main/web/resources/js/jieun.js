...
jieun=(()=>{
	var w,nav,footer,content,context,script,style,img;
		...
		var detail=()=>{
				...
				$('<span />').append($('<strong/>').html('61%').attr({id:'je_line',class:'je_discount'})).appendTo(p_price);
				let price=$('<span/>').attr({class : 'je_price'});
				let del = $('<del/>').addClass('je_origin_cost').text('299,000');
				let ins = $('<ins/>').addClass('je_dis_cost').text('119,000');
				$('<br/>').appendTo(del);
